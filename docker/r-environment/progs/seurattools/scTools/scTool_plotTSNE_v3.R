scTool_PlotTSNE.v3 <- function(seurat.obj, npcs = 30, id=NULL, umap.f=NULL, tsne.f=NULL, 
                               reductiontype="pca", clustname="RNA_snn_res.1", 
...
seurat.obj <- SetIdent(object = obj, value = clustname)
  
# Plotting tSNE 2D

png(filename = paste0(tsne.f,"2D/",id, "_tSNE_",clustname, ".png"), units = "in", width = 12, height = 9, res = 200)
print(DimPlot(obj, pt.size = pointsize, label = T, label.size = labsize, reduction="tsne") + 
...
print(DimPlot(object = obj, reduction="umap", pt.size = pointsize, label = T, label.size = labsize) +
